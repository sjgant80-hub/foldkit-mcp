#!/usr/bin/env node
// foldkit-mcp · stdio MCP server exposing foldkit as Claude-callable tools + resources
...
const server = new Server(
  { name: 'foldkit-mcp', version: '1.0.0' },
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  try {
    ...
    return {
      isError: true,
      content: [{ type: 'text', text: `foldkit-mcp error: ${err.message}` }]
    };
  }
});
